...
describe('`map.has()` indicates whether an element with a key exists', function() {
	...
	it('finds an element by its key', function() {
	...
	it('does not coerce keys', function() {
		let map = new Map([[1, 'one'], ['1', 'one, but as a string']]);
	...
	it('adding an item (using `map.set(key, value)`) later will make `has()` return true', function() {
		let map = new Map();
		map.set(undefined, 'a value for the key undefined');
		map.set(); // also works, with the value undefined
